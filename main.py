...
def draw_shell_ui(keys):
    ...
    total_bar_height = 50 * 5  # 각 상태바 간격이 50px, 총 5개
    top_bottom_margin = 20     # 위아래 여백 고려
    status_bg_height = total_bar_height + top_bottom_margin * 2

    status_bg_y = egg_center_y - status_bg_height // 2

    
    manual_box_w = status_bg.get_width()
    manual_box_h = 60
    manual_box_x = status_bg_x
    manual_box_y = status_bg_y - manual_box_h - 50  # 상태바 위에 여백 20
    ...
    text_rect = manual_text.get_rect(center=(manual_box_x + manual_box_w // 2, manual_box_y + manual_box_h // 2))
    screen.blit(manual_text, text_rect)
